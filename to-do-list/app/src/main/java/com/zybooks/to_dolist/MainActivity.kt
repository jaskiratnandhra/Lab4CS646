...
class MainActivity : AppCompatActivity(), SensorEventListener {
    ...
    private fun displayList() {
        ...
        for (i in items.indices) {
            itemText.append(" - ").append(items[i]).append(lineSeparator)
        }
    ...
    private fun clearButtonClick() {
        ...
        val moveBoardOff = ObjectAnimator.ofFloat(
            listTextView, "translationY", -screenHeight)
        moveBoardOff.duration = 600
        moveBoardOff.start()
        ...
        moveBoardOff.addListener(object : AnimatorListenerAdapter() {
            override fun onAnimationEnd(animation: Animator) {
                ...
                val moveBoardOn = ObjectAnimator.ofFloat(
                    listTextView, "translationY", screenHeight, 0f)
                moveBoardOn.duration = 600
                moveBoardOn.start()
            }
        })
    }
